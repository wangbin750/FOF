...
def Risk_Budget_Weight(cov_mat, budg_list):
    '''
    计算风险平价配置权重
    :param cov_mat: 资产的方差协方差矩阵-DataFrame
    :budg_list: 风险预算向量-list
    :return: 配置权重-Series
    '''
    ...
    omega = np.matrix(cov_mat.values)
    budget = np.array(budg_list)

    def fun(x):
        tmp = (omega * np.matrix(x).T).A1
        risk = x * tmp
        risk_all = np.matrix(x) * (omega * np.matrix(x).T)
        bud = risk_all * budget
        delta_risk = np.sum(np.array(risk - bud)**2)
        return delta_risk
